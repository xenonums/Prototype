...
class log_in(View):
    ...
    def post(self, request):
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            ...
            n = User.objects.filter(username=name).count()
            if n == 1:
                ...
                return JsonResponse({'msg': "激活码已发送"})
        ...
        if 'rg' in request.POST:
            ...
            try:
                cod = Code.objects.get(name=usn).code
            ...
            if n == 1:
                ...
                messages.error(request, msg)
                return render(request, "login.html")
            elif cod != code or cod is None or Code.objects.get(name=usn).email != email:
                msg = "激活码与用户名、邮箱不匹配"
                messages.error(request, msg)
                return render(request, "login.html")
            else:
